...
public class Map {
	...
	/**
	 * Given a piece it will place it in the map. If it overlaps it will return
	 * false. If successful it will return true. Object will snap to the closest
	 * shape
	 * 
	 * @param piece
	 * @return success?
	...
	public boolean put(BasicTriangle piece) {
		...
		for (int yOffset = coords[1].YCoord(); yOffset <= Math.abs(vertical); yOffset++) {
			int horizontal = (int) ((yOffset * Math.tan(Math.atan(lengths[0]
					/ ((double) lengths[1])))) + .5)
					+ coords[1].XCoord();
			for (int xOffset = coords[1].XCoord(); xOffset <= Math
					.abs(horizontal); xOffset++) {
				if (!map[yOffset][xOffset])
					map[yOffset][xOffset] = true; // Works as long as a and b
		...
		for (int yOffset = coords[1].YCoord(); yOffset <= Math.abs(vertical); yOffset++) {
			...
			for (int xOffset = coords[1].XCoord(); xOffset <= Math
				...
				if (map[yOffset][xOffset] && !(x == xOffset && y == yOffset))
					map[yOffset][xOffset] = false; // Works as long as a and b
													// have same x coordinate
				else
					return;
	...
	/**
	 * Given a piece to remove it will take it out of the map. Returns false if
	 * it is not in the map or if operation fails
	 * 
	 * @param piece
	 * @return success?
	 */
	public boolean take(BasicTriangle piece) {
		...
		for (int yOffset = 0; yOffset <= Math.abs(vertical); yOffset++) {
			int horizontal = (int) (vertical * Math.tan(Math.atan(lengths[0]
					/ ((double) lengths[1]))));
			for (int xOffset = 0; xOffset <= Math.abs(horizontal); xOffset++) {
				if (!map[yOffset + vertical + coords[1].YCoord()][xOffset
						+ horizontal + coords[1].XCoord()])
					map[yOffset + vertical + coords[1].YCoord()][xOffset
							+ horizontal + coords[1].XCoord()] = true; // Works
																		// as
																		// long
																		// as a
																		// and b
